...
export class SuscripcionComponent implements OnInit {
  ...
  cargarDatos(): void {
    this.http.get<PlanSuscripcion[]>('http://localhost:8081/plan/getall').subscribe({
      next: (planes) => {
        this.planes = planes ?? [];
  ...
  cargarSuscripcionActiva(): void {
    this.http.get<Suscripcion>('http://localhost:8081/suscripcion/miSuscripcion').subscribe({
      next: (data) => {
  ...
  cancelarSuscripcion(): void {
    if (!confirm('¿Estás segura de que deseas cancelar tu suscripción?')) return;
    this.procesando = true;
    this.http.put(
      ...
      error: (err) => {
        this.error = err.error || 'Error al cancelar.';
        this.procesando = false;
        this.cdr.detectChanges();
      }
    });
